...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            _, h_old, w_old = img_a.size()
            h_pad = (h_old // 8 + 1) * 8 - h_old
            w_pad = (w_old // 8 + 1) * 8 - w_old
            img_a = torch.cat([img_a, torch.flip(img_a, [1])], 1)[:, :h_old + h_pad, :]
            img_a = torch.cat([img_a, torch.flip(img_a, [2])], 2)[:, :, :w_old + w_pad]
            img_b = torch.cat([img_b, torch.flip(img_b, [1])], 1)[:, :h_old + h_pad, :]
            img_b = torch.cat([img_b, torch.flip(img_b, [2])], 2)[:, :, :w_old + w_pad]
