...
def main():
    ...
    parser.add_argument('--model_path', type=str,
                        default='./Model/Fusion/Fusion/models')
    parser.add_argument('--iter_number', type=str,
                        default='40800')#198epoch
    ...
    # set up model
    model_path = os.path.join(args.model_path,'Best.pth')
    ...
    a_dir="./data/test/SPECT/MRI"
    b_dir="./data/test/SPECT/SPECT_Y"

    save_dir = "./results/SPECT-MRI/"
    ...
    for i, test_data in enumerate(test_loader):
        ...
        img_a = img_a.permute(0, 3, 1, 2).float()
        img_b = img_b.permute(0, 3, 1, 2).float()#torch.Size([1, 1, 480, 640])
        start=time.time()
        # inference
        with torch.no_grad():
            ...
            output = torch.where(output > ones, ones, output)
            output = torch.where(output < zeros, zeros, output)
...
def test(img_a, img_b, model, args, window_size=None):
